...
姓名：邱志翔  学号：2312190432  角色：前端  日期：2026-04-28
...
- [x] 覆盖登录、注册、社区发帖、AI 文案辅助、识别结果发布、请求失败回退、Mock 数据契约等核心前端流程

### 覆盖率

- 运行命令：`npm test`
- 覆盖率命令：`npm test -- --coverage`
- 本地终端总覆盖率：行覆盖率 `94.56%`，分支覆盖率 `76.60%`，函数覆盖率 `90.29%`
- 业务代码覆盖率：`92.77%`（基于 `frontend/coverage/coverage-summary.json`，用于 Codecov / LCOV 上传）
...
- PR #38: https://github.com/hzy2005/AviAI/pull/38
...
2. 问题：社区页面同时包含搜索、发帖、AI 文案、图片上传、编辑和删除等多种状态，直接测试耦合较高。  
解决：拆成“页面交互测试”“API / Mock 测试”“覆盖率提升测试”三组文件，把上传、AI 文案、鉴权、失败回退、分页加载、编辑删除等流程分别断言。
...
4. 问题：终端覆盖率和 Codecov 覆盖率口径不完全一致。  
解决：在贡献说明中同时记录 Node 终端总覆盖率和 `coverage-summary.json` 的业务代码覆盖率，明确两者用途不同，避免验收时混淆。

## 心得体会

这次测试作业让我更清楚地意识到，前端测试不只是“点页面有没有反应”，而是要把输入校验、接口调用、异常提示、状态切换这些细节都固定下来。对微信小程序项目来说，只要把 `wx` 能力和页面对象抽象好，一样可以建立比较稳定的自动化测试体系。通过这次补测，我也更直观地体会到覆盖率提升不是单纯追数字，而是把社区发帖、AI 文案辅助、请求失败回退这些真实风险点逐步固化成可重复执行的断言，这样后续改功能时才能更快发现回归问题。
